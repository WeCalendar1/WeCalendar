...
type TagCreatorInlineProps = {
  ...
  /** Prefill and open immediately (edit mode) */
  initial?: { name: string; color: string };
  onCancel?: () => void;
  submitLabel?: string;
};

export function TagCreatorInline({
  onAdd,
  initial,
  onCancel,
  submitLabel,
}: TagCreatorInlineProps) {
  ...
  const primaryLabel = submitLabel ?? (isEdit ? "Save" : "Add");
  ...
  async function handleAdd() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : isEdit ? "Could not update tag." : "Could not create tag.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2">
        ...
        >
          {busy ? (isEdit ? "Saving…" : "Adding…") : primaryLabel}
        </button>
        <button
          type="button"
